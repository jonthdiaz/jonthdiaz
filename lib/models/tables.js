...
class tables{
  ...
  createTableContact(connection, callback){
    rethinkdb.db(config.db_name).tableCreate('contact').run(
      connection, (err, result)=>{
      if(err) console.log("tabla contact already created")
      else console.log("created new table contact")
      callback(null, connection)
    })
  }
}
