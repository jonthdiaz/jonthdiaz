...
var _contact = require('../schema/contact');
...
var contact = function () {
  ...
  _createClass(contact, [{
    ...
    value: function getAll(callback) {
      ...
      _async2.default.waterfall([_db2.default.connect, function (connection, callback) {
        _rethinkdb2.default.table(_this.tableName).filter({ active: 'on' }).orderBy('order').run(connection, function (error, cursor) {
          connection.close();
          if (error) return callback(true, 'Error fetching model ' + _this.tableName);
          cursor.toArray(function (error, result) {
            if (error) return callback(true, 'error reading cursor  ' + _this.tableName);
    ...
    value: function getById(id, callback) {
      ...
      _async2.default.waterfall([_db2.default.connect, function (connection, callback) {
        _rethinkdb2.default.table(_this2.tableName).get(id).run(connection, function (error, result) {
          if (error) return callback(true, 'Error getting register ' + _this2.tableName + ' ' + error.message);
          callback(null, result);
        });
      }], function (error, data) {
        callback(error === null ? false : true, data);
    ...
    value: function update(id, data, callback) {
      ...
      _async2.default.waterfall([_db2.default.connect, function (connection, callback) {
        if (!id) return callback(true, 'The register with id ' + id + ' does not exists table ' + _this3.tableName);
        var model = new _contact2.default(data);
    ...
    value: function _delete(id, callback) {
      ...
      _async2.default.waterfall([_db2.default.connect, function (connection, callback) {
        if (!id) return callback(true, 'The register with id ' + id + ' does not exists on table ' + _this5.tableName);
        _rethinkdb2.default.table(_this5.tableName).get(id).delete().run(connection, function (error, result) {
          connection.close();
          if (error) return callback(true, 'Error happens while deleting record ' + _this5.tableName);
          callback(null, result);
        });
      }], function (error, data) {
        callback(error === null ? false : true, data);
